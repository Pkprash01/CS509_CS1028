...
1. **GEMM (General Matrix Multiplication):** Simple direct nested-loop implementation and Cache-blocking implementation
2. **CSR (Compressed Sparse Row) Graph Representation:** Preprocessing module converting graph adjacency lists into compact CSR arrays (`row_ptr`, `col_idx`, `values`)
...
└── Assignment_01/
    ...
    │   └── input.txt                   <-- Sample test matrix input
    └── outputs/
        ├── csr_output.txt              <-- Generated CSR output arrays
        ...
        ├── input_out.txt               <-- Matrix output log
...
## Assignment 01: Individual Tasks

### 1. GEMM (General Matrix Multiplication)

#### Objective
Compute the matrix product $C = A \times B$ using two implementations:
1. **Simple GEMM:** Direct 3-nested loop matrix multiplication.
2. **Blocking GEMM:** Tiled iteration designed to maximize CPU L1/L2 cache locality.
...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| `input.txt` | 2x3 & 3x2 | 2x2 Matrix | Matches Expected | 0.0000 ms| 0.0000 ms | 16x16 | Pass |
...
| `gemm_500.txt` | 500x500 | 500x500 Matrix | Matches Expected | 435.8110 ms |544.1910 ms | 32x32 | Pass |
| `gemm_1000.txt` | 1000x1000 | 1000x1000 Matrix | Matches Expected | 4808.6830 ms |3557.4890 ms | 32x32 | Pass |
---

### 2. CSR Graph Representation

#### Objective
Convert graph adjacency list inputs into Compressed Sparse Row (CSR) array representations.
...
#### Preprocessing Rule
Adjacency-list-to-CSR conversion is executed as **preprocessing setup**
Its runtime is omitted from algorithm execution timing as per lab 
...
| Single | `csr_input_100.txt` | 100 |  | Adjacency List | Converted to CSR | 10.624 ms | Pass |
| Single | `csr_input_1000.txt` | 1,000 | 2,000 | Adjacency  | Converted to CSR | 13.246 ms | Pass |
| Single | `csr_input_10000.txt` | 10,000 | 20, | Adjacency List | Converted to CSR | 32.607 ms | Pass |
...
## References
1. CS509 Assignment 1 Specification Document.
2. CS509 M.Tech Lab Work Guidelines.